...
export default function KolPage() {
  ...
  const loadKols = React.useCallback(async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Không tải được danh sách KOL', description: err.message });
    } finally {
      setIsLoading(false);
    }
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Tìm không được', description: err.message });
    } finally {
      setAddSearching(false);
    }
  };

  const pickUserToPromote = (u: User) => {
    setAddOpen(false);
    setAddPhone('');
    setAddResults([]);
    openPromote(u.id, u.name || u.phone || u.id.slice(0, 8));
  };
                ...
                <TableRow key={r.userId}>
                  ...
                  <TableCell className="text-sm">
                    {r.kind === 'STANDARD' && r.leaderId
                      ? (leaderName.get(r.leaderId) ?? r.leaderId.slice(0, 8))
                      : <span className="text-muted-foreground">—</span>}
                  </TableCell>
...
// The API client throws Error(JSON.stringify(errorData)); surface the human message when present.
