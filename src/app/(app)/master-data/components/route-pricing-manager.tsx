                    ...
                    {isPricingLoading ? (
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {pricing.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={4} className="h-24 text-center">No pricing rules found for this route.</TableCell>
                                    </TableRow>
                                ) : (
                                    pricing.map(p => (
                                        <TableRow key={p.id}>
...
function PricingForm({
    ...
    const districtOptions = React.useMemo(() => districts.map(d => ({
        value: String(d.id),
        label: d.name,
    })), [districts]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEditing) {
                ...
                await createPricing({ routeId, adminUnitId, price: Number(price), priority: 1 });
