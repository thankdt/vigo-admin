...
- **`system-config` — SỬA lại theo code thật:** endpoint là **`POST /system-config` một-key** `{ key, value, description }` (`master-data.controller.ts:151-156`; FE `updateSystemConfig(key,...)` `api.ts:909`) — **KHÔNG có** bulk-PATCH nhiều key. Vì mỗi request đúng 1 key → gate **chính key đó** theo `settings.<groupIdFor(key)>`. `groupIdFor` **hiện chỉ có ở FE** (`system-config-groups.ts:38`) → **phải port sang BE** (net-new: nhân đôi bảng prefix/label + test đồng bộ 2 phía). `GET /system-config` gate **any-of `settings.*`**.
  ...
  - **✅ Audit ĐÃ CHẠY (kết quả):** quét 170 migration + `zalo.service` → `system_config` **CÓ secret thật**: `ZALO_ACCESS_TOKEN`, `ZALO_REFRESH_TOKEN` (seed `1767700000000-SeedZaloTokens.ts`; `zalo.service.ts:10-12` tự rotate runtime). Các key khác đều là tham số vận hành không-bí-mật. → **Quyết định: `GET system-config` REDACT secret bằng PATTERN + list** (`_TOKEN$/_SECRET$/PASSWORD/CREDENTIAL/PRIVATE_KEY/WEBHOOK_SECRET/_API_KEY$` + list phụ) — **tự phủ secret tương lai** đặt tên theo quy ước, không phải hardcode cứng; hôm nay chỉ ZALO_* khớp. Không trả value ra bất kỳ ai + chặn cả sửa. Read-all + FE-hide **giữ nguyên** cho key thường. `zalo.service` đọc token trực tiếp từ DB nên redact KHÔNG hỏng Zalo. (Không dời ra env đợt này để tránh đụng service đang chạy; secret MỚI nên để env.)
